...
Theme themes[] = {
    // 256-color explicit mapping: Green=46, Yellow=226, Red=196, Magenta=13, Blue=12, Cyan=14, Grey=244
...
struct {
    ...
    .display_refresh = 2,
    .test_mode = 0,
    .state_file = "/tmp/syncmon_state.env",
    .config_file = "./ressources/syncmon.conf",
    .dash_w = 92,
    .panel_l = 42,
    .panel_r = 48,
    .use_braille = 1
...
// Safely handles empty environment variables and pointer overlap
...
void load_state() {
    ...
    if (config.test_mode) {
        ...
        snprintf(state.r_det, sizeof(state.r_det), "link=%s io=%d host=172.31.%d.%d", r_link, rand_range(0,14), rand_range(0,254), rand_range(0,254));
        ...
        if (access(config.config_file, R_OK) == 0) parse_env_file(config.config_file);

        // Fallbacks are now solid static defaults preventing blank space corruption
        ...
        // Use global update timestamp as the ultimate fallback for sub-checks
...
void draw_graph(int x, int y, int* hist_data, int head, int max_w, Theme* th) {
    for (int c = 0; c < max_w; c++) {
        int points_back = (max_w - 1 - c);
        int idx = head - 1 - points_back;
        while (idx < 0) idx += HIST_SIZE;
        idx %= HIST_SIZE;

        int v = hist_data[idx];
        if (v == -1) {
            tb_set_cell(x + c, y, ' ', th->fg, th->bg);
            continue;
        }

        // Handle highly visible error marks using color inversion
        if (v == 0) {
            // Draw an 'X' using the background color, and flood the cell background with the error color
            tb_set_cell(x + c, y, 'X', th->bg | TB_BOLD, th->err);
            continue;
        }

        uint16_t color = th->ok;
        if (v == 2) color = th->warn;

        if (config.use_braille) {
            uint32_t ch = braille_bar(v, v);
...
void format_shortened_left(char *buf, size_t buf_sz, const char* prefix, const char* val, int max_disp_len) {
    ...
    if (pref_len + val_len <= max_disp_len) {
        ...
        int keep_len = max_disp_len - pref_len - 3; // 3 for "..."
        if (keep_len < 0) {
...
void draw_ui(int anim_tick) {
    ...
    tb_print_custom(bx+bw-4-strlen(buf), y+1, th->fg, th->bg, buf);
...
int main(int argc, char** argv) {
    ...
    for (int i=1; i<argc; i++) {
        if (strcmp(argv[i], "--test") == 0) config.test_mode = 1;
            ...
            printf("Usage: syncmon-modern [--test] [--no-braille] [--help]\n");
            return 0;
        }
    }

    const char* env_ref = getenv("DISPLAY_REFRESH");
    if (env_ref) config.display_refresh = atoi(env_ref);
    if (config.display_refresh <= 0) config.display_refresh = 2;

    const char* env_braille = getenv("USE_BRAILLE");
    if (env_braille) config.use_braille = atoi(env_braille);

    const char* env_st = getenv("STATE_FILE");
    if (env_st) strncpy(config.state_file, env_st, sizeof(config.state_file)-1);
    ...
    while (keep_running) {
        ...
        if (now - last_update >= config.display_refresh * 1000) {
            load_state();
            last_update = now;
        }
            ...
            if (theme_menu_open) {
                if (ev.type == TB_EVENT_KEY) {
                    if (ev.key == TB_KEY_ARROW_UP) {
                        theme_menu_sel = (theme_menu_sel - 1 + num_themes) % num_themes;
                    } else if (ev.key == TB_KEY_ARROW_DOWN) {
                        theme_menu_sel = (theme_menu_sel + 1) % num_themes;
                    } else if (ev.key == TB_KEY_ENTER) {
                        current_theme = theme_menu_sel;
                        theme_menu_open = 0;
                ...
                if (ev.type == TB_EVENT_KEY) {
                    if (ev.ch == 'q' || ev.key == TB_KEY_CTRL_C) {
                        break;
                    } else if (ev.ch == 'g') {
                        config.use_braille = !config.use_braille;
                    } else if (ev.ch == 't') {
                        theme_menu_sel = current_theme;
                        theme_menu_open = 1;
                    }
                }
            }
