...
#undef LOAD_COMP
    ...
    char chk_stat[16];
    derive_check_status(state.lb_check, chk_stat, sizeof(chk_stat));
    hist_lb_status[hist_idx] = status_to_val(chk_stat);

    derive_check_status(state.nc1_check, chk_stat, sizeof(chk_stat));
    hist_nc1_status[hist_idx] = status_to_val(chk_stat);

    derive_check_status(state.nc2_check, chk_stat, sizeof(chk_stat));
    hist_nc2_status[hist_idx] = status_to_val(chk_stat);

    derive_check_status(state.nfs_check, chk_stat, sizeof(chk_stat));
    hist_nfs_status[hist_idx] = status_to_val(chk_stat);

    derive_check_status(state.dns_check, chk_stat, sizeof(chk_stat));
    hist_dns_status[hist_idx] = status_to_val(chk_stat);
 ...
 */
static void draw_box_wave(int x, int y_border, int box_w, uint16_t fg, uint16_t bg,
                          int wave_pos)
{
...
void draw_status_graph(int x, int y, int* hd, int head, int mw, Theme* th) {
    for (int c = 0; c < mw; c++) {
        ...
        if (v == 4)      { h = 8; col = th->ok; }
        else if (v == 2) { h = 6; col = th->warn; }
        else if (v == 0) { h = 2; col = th->err; }
        else             { h = 1; col = th->skip; }

        if (config.use_braille) {
            uint32_t b = 0x2800;
            if (h >= 1) b |= 0x40; // bottom left
            if (h >= 2) b |= 0x80; // bottom right
            if (h >= 3) b |= 0x04; // middle-low left
            if (h >= 4) b |= 0x20; // middle-low right
            if (h >= 5) b |= 0x02; // middle-high left
            if (h >= 6) b |= 0x10; // middle-high right
            if (h >= 7) b |= 0x01; // top left
            if (h >= 8) b |= 0x08; // top right
            tb_set_cell(x+c, y, b, col, th->bg);
        } else {
            static const uint32_t blocks[] = {
                0x2508, 0x2581, 0x2582, 0x2583, 
                0x2584, 0x2585, 0x2586, 0x2587, 0x2588
            };
            tb_set_cell(x+c, y, blocks[h], col, th->bg);
        }
    }
}
 ...
 *
 * Wave animation: while the dot is inside the tunnel both the upper and bottom 
 * borders of the status box pulse with a travelling sine-like wave.
 */
                                  ...
                                  Theme* th, int tick)
{
    ...
    /* ── bottom-border and upper-border wave while dot is inside the tunnel ── */
    if (in_tunnel && tunnel_delay > 0) {
        ...
        draw_box_wave(box_x, box_y, box_w, col, th->bg, wave_interior_pos + 1);
    }
...
void draw_ui(int anim_tick) {
    ...
    tb_print_custom(bx+4, y+3, th->fg, th->bg, "LB status  :");
    draw_status_graph(bx+19, y+3, hist_lb_status, hist_idx, bw-21, th);

    tb_print_custom(bx+2, y+4, th->highlight, th->bg, SYM_NC);
    tb_print_custom(bx+4, y+4, th->fg, th->bg, "NC1 status :");
    draw_status_graph(bx+19, y+4, hist_nc1_status, hist_idx, bw-21, th);

    tb_print_custom(bx+2, y+5, th->highlight, th->bg, SYM_NC);
    tb_print_custom(bx+4, y+5, th->fg, th->bg, "NC2 status :");
    draw_status_graph(bx+19, y+5, hist_nc2_status, hist_idx, bw-21, th);

    tb_print_custom(bx+2, y+6, th->highlight, th->bg, SYM_NFS);
    tb_print_custom(bx+4, y+6, th->fg, th->bg, "NFS status :");
    draw_status_graph(bx+19, y+6, hist_nfs_status, hist_idx, bw-21, th);

    tb_print_custom(bx+2, y+7, th->highlight, th->bg, SYM_DNS);
    tb_print_custom(bx+4, y+7, th->fg, th->bg, "DNS status :");
    draw_status_graph(bx+19, y+7, hist_dns_status, hist_idx, bw-21, th);
...
int main(int argc, char** argv) {
    for (int i = 0; i < HIST_SIZE; i++) {
        ...
        hist_lb_status[i] = hist_nc1_status[i] = hist_nc2_status[i] = hist_nfs_status[i] = hist_dns_status[i] = -1;
    }
