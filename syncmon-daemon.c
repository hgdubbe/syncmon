...
void write_state(const char* state_file, const char* status,
...
void get_mysql_gtid(const char* host, const char* port, const char* user,
    ...
    if (!exec_query(cmd, out, 256) || strlen(out) == 0) {
        snprintf(cmd, sizeof(cmd),
                 "timeout 10 mysql -h %s -P %s -u %s %s -N -B -e 'SHOW GLOBAL VARIABLES LIKE \"%s\";' 2>/dev/null | awk '{print $2}'",
                 host, port, user, pass_opt, var);
...
void run_checks(Config *cfg) {
    ...
    if (cfg->enable_mysql_check) {
        ...
        if (master_ok && slave_ok) {
            if (strlen(mysql_master_gtid) > 0 && strcmp(mysql_master_gtid, mysql_slave_gtid) == 0)
    ...
    write_state(cfg->state_file, overall,
...
void run_test_mode(const char* state_file) {
    ...
    write_state(state_file, statuses[rand() % 3],
                statuses[rand() % 3], statuses[rand() % 3],
                m_gtid == s_gtid ? "OK" : "WARN",
                m_gtid_str, s_gtid_str,
                statuses[rand() % 3], statuses[rand() % 3],
                rep_status, r_detail, "Live Simulation Data", ts_str, ts_str);
...
int main(int argc, char *argv[]) {
    ...
    /* Ensure log directory exists before loading config / writing logs */
